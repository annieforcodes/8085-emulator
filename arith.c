...
static uint8_t read_reg(CPUState *cpu, uint8_t code){
    uint8_t data;
     switch(code){
        case 0:data=cpu->b; break;//v2printf("data read from b\n"); break;
        case 1:data=cpu->c; break;//v2printf("data read from c\n"); break;
        case 2:data=cpu->d; break;//v2printf("data read from d\n"); break;
        case 3:data=cpu->e; break;//v2printf("data read from e\n"); break;
        case 4:data=cpu->h; break;//v2printf("data read from h\n"); break;
        case 5:data=cpu->l; break;//v2printf("data read from l\n"); break;
        case 6:
             {
                uint16_t address = (((uint16_t)cpu->h)<<8)|cpu->l;
                data = memory[address];
                //v2printf("data read from m\n"); 
                break;
             }
        case 7:data=cpu->a; //v2printf("data read from a\n"); break;
            }
        return data;
...
cpu->a = (uint8_t)sum;
cpu->pc +=1;
//v2printf("executed ADD with result 0x%02X in acc\n", cpu->a);
...
if(cpu->a < reg_data){ // borrow corrected after randomized verification
    cpu->f |= (1<<0); // set carry flag
}
//AC (carry from bit 3 to 4)
...
cpu->a = (uint8_t)sum;
cpu->pc +=1;
//v2printf("executed SUB with result 0x%02X in acc\n", cpu->a);
