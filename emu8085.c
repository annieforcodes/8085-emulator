...
int main(int argc, char *argv[]){
    //CPUState cpu = {0}; zero all regs  changed in v2
    cpu.pc = 0;
    /* ~~ removed to facilitate automation~~~~ v2
    cpu.h = 0x00;
    cpu.l = 0xAF;
    //other storage 
    ...
    if(argc < 2){
        printf("Usage: ./emulator program.hex\n");
        return 1;
    }
    FILE *fp = fopen(argv[1],"r");
    if(!fp){
        printf("error can't open program.hex\n");
        return 1;
    }
    int byte;
    int addr = 0;

    while(fscanf(fp,"%x",&byte)== 1){
        memory[addr++] = (uint8_t)byte;
    }
    fclose(fp);
